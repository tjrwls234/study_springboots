...
public class DataInfors {
    ...
    public BoardBean getDataByUid(String uid) {
        ArrayList<BoardBean> boardList = getDataListWithBoardBean();
        BoardBean boardBean;
        if ("UID1".equals(uid)) {
            boardBean = boardList.get(0);
        } else if ("UID2".equals(uid)) {
            boardBean = boardList.get(1);
        } else if ("UID3".equals(uid)) {
            boardBean = boardList.get(2);
        } else {
            boardBean = boardList.get(3);
        }
        return boardBean;
    }
}
